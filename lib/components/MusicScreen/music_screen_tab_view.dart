  ...
  Future<void> _getPage(int pageKey) async {
    try {
      final newItems = await _jellyfinApiHelper.getItems(
        ...
        includeItemTypes: widget.tabContentType.includeItemTypes,
            ...
            if (widget.searchTerm == null) {
              if (widget.tabContentType == TabContentType.songs) {
                ...
                offlineSortedItems = downloadsHelper.downloadedParents
                    .where((element) =>
                        element.item.type ==
                            widget.tabContentType.includeItemTypes &&
                        element.viewId ==
                            _finampUserHelper.currentUser!.currentViewId)
                    .map((e) => e.item)
                    .toList();
...
bool _offlineSearch(
  ...
  return item.type == tabContentType.includeItemTypes && containsName;
}
